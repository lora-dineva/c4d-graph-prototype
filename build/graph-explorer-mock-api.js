 ...
 * over the inlined GRAPH (graph-explorer-data.js). Method bodies are pure
 * graph traversal — swap them for fetch() later and the UI never changes.
 *
 * Depends on globals: GRAPH, ALERTS, ALERT_HISTORY, SYSTEM_VIEWS, SCENARIOS.
 * MUST be INLINED into index.html AFTER the data <script> block.
 *
 * Forward-traversal whitelist (blast radius): follow an edge downstream iff
 *   relationship ∈ { SUPPLIES, PART_OF_ASSEMBLY, USED_IN, ASSEMBLED_AT,
 *                    MATERIAL_DEPENDENCY }
 *   AND NOT (relationship==SUPPLIES && target is a supplier)   // skip business edges
 * Reverse traversal (program → upstream suppliers) inverts the same whitelist.
 * ===========================================================================*/
...
(function (global) {
  ...
  // ---- indexes -------------------------------------------------------------
  const N = new Map();                 // id -> node.data
  const OUT = new Map();               // id -> [edge.data]  (source==id)
  const IN  = new Map();               // id -> [edge.data]  (target==id)
  function indexGraph() {
    N.clear(); OUT.clear(); IN.clear();
    GRAPH.nodes.forEach(n => { N.set(n.data.id, n.data); OUT.set(n.data.id, []); IN.set(n.data.id, []); });
  ...
  const FWD = new Set(["SUPPLIES","PART_OF_ASSEMBLY","USED_IN","ASSEMBLED_AT","MATERIAL_DEPENDENCY"]);

  // follow downstream? (blast radius)
  function followFwd(e) {
    if (!FWD.has(e.relationship)) return false;
    if (e.relationship === "SUPPLIES" && isSupplier(e.target)) return false; // skip business T2→T1 / T3→T2
    return true;
  }

  // simulate async latency so the UI's loading states are exercised
  const reply = (val, ms = 60) => new Promise(res => setTimeout(() => res(val), ms));

  // ---- core traversals -----------------------------------------------------

  // forward reachable set from a source (downstream toward programs/plants)
  ...
  // upstream supplier set reachable from a program (reverse of FWD whitelist)
  function upstreamSuppliers(programId) {
    const seen = new Set([programId]), suppliers = new Set(), q = [programId];
    while (q.length) {
      const cur = q.shift();
      (IN.get(cur) || []).forEach(e => {
        // accept the FWD relationship set plus business supplier→supplier SUPPLIES (we want the full deep set)
  ...
  // ---- per-path enumeration for the Impact summary / Path Index -----------
  // canonical reconciled per-path figures (match SCENARIOS.A_blast_radius)
  ...
  const mockApi = {
    ...
    getGraph() {
      return reply({
        ...
        meta: { counts: {
          suppliers: GRAPH.nodes.filter(n => n.data.type === "supplier").length,
          parts:     GRAPH.nodes.filter(n => n.data.type === "part").length,
          plants:    GRAPH.nodes.filter(n => n.data.type === "plant").length,
          programs:  GRAPH.nodes.filter(n => n.data.type === "program").length,
    ...
    traceImpact(sourceId) {
      ...
      const plantIds   = ids.filter(i => node(i)?.type === "plant");
      ...
      const first = paths.reduce((m,p) => (p.bufferDays < m.bufferDays ? p : m), { bufferDays: Infinity });

      // alternative analysis for any SPOF part: qualified-but-correlated counts as none-independent
      const altEdges = GRAPH.edges.filter(e => e.data.relationship === "QUALIFIED_FOR" && partIds.includes(e.data.target));
      ...
      return reply({
        ...
        paths,
        healthyContrast: SCENARIOS.healthy_control
      });
    },

    // ── Q2 — concentration leaderboard ───────────────────────────────────
    // A supplier "takes down" program P iff it is MANDATORY (a sole-source link on
    // a chain of single-source edges) to a SPOF part used by P. Suppliers with a
    // substitutable input (≥2 providers, e.g. SilTech's two wafer sources) are NOT
    // mandatory and do not propagate. Reproduces SCENARIOS.Q2_concentration_leaderboard.
    getConcentration() {
      const spofParts = GRAPH.nodes
        .filter(n => n.data.type === "part" && n.data.single_point_of_failure)
        .map(n => n.data.id);
      const scoreMap = {}; // supplierId -> Set(programIds)
    ...
    // ── Q3 — hidden shared deep dependency ───────────────────────────────
    getSharedDependencies(programIds) {
      ...
      // rank: deepest tier first (Tier-3 convergence is the headline)
    ...
    // ── Q4 — ownership chain + concentration by owner ────────────────────
    ...
    // ── Q5 — alternative path for a part ─────────────────────────────────
    ...
    // ── Q9 — change simulation (re-source a part) ────────────────────────
    simulateResource(partId, newSupplierId) {
      ...
      // does new supplier already feed those programs? -> new concentration
    ...
    search(q) {
      ...
      const all = GRAPH.nodes.map(n => n.data);
    ...
    // ── saved views (localStorage best-effort + in-memory) ───────────────
    listViews() {
      let mine = [];
      try { mine = JSON.parse(localStorage.getItem("ge_my_views") || "[]"); } catch (e) {}
      return reply({ system: SYSTEM_VIEWS, mine });
    },
    saveView(view) {
      ...
      } catch (e) { /* file:// may block localStorage — view still returned for the session */ }
  ...
  function mkAlt(supplierId, partId, qualifiedOnly, edge) {
    ...
    return {
      ...
      independent: !qualifiedOnly ? true : !correlated   // a qualified alt that shares owner is NOT independent
    };
  }

  // suppliers that are MANDATORY (single-source chain) to a given SPOF part
  function mandatorySuppliersFor(spofPartId, acc) {
    ...
    if (direct.length === 1) {            // exactly one supplier => mandatory
      const sid = direct[0].source;
      result.add(sid);
      climbMandatory(sid, result);
    }                                     // ≥2 suppliers => dual-sourced, none individually mandatory
    // material dependencies (e.g. shared die) are required inputs => their sole suppliers are mandatory too
    (IN.get(spofPartId) || [])
  ...
  function climbMandatory(supplierId, result) {
    ...
    if (inputs.length === 1) {            // single upstream input => still mandatory
      const u = inputs[0].source;
      if (!result.has(u)) { result.add(u); climbMandatory(u, result); }
    }                                     // ≥2 inputs => substitutable, stop propagating
  }
  ...
  global.geReindex = indexGraph; // call if GRAPH mutated at runtime
